...
const BASE_API_URL = 'http://localhost:8080'; // Change this to your backend API URL
...
const userId = "64c29638f109d100643aa5f1";
...
const insertLocations = async () => {
    try {
        for (const locationName of locations.map((location) => location.name)) {
            ...
            const response = await axios.post(`${BASE_API_URL}/api/locations`, {
                ...locationData
            });
            ...
            if (response) {
                ...
                console.error(`Failed to insert location "${locationName}". Error:`, response.message);
            }
        }
